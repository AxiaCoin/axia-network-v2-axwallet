...
class Bitcoin implements Currency {
    ...
    data.forEach((e) {
      ...
      transactions.add(
        TransactionItem(
          isInput: isInput,
          from: fromAddress,
          fromTotal: inputs.length,
          to: toAddress,
          toTotal: outputs.length,
  ...
  Future sendTransaction(double amount, String receiverAddress) async {
    ...
    try {
      ...
      if (amount == availBal) {
        txb.addOutput(receiverAddress, BigInt.from(amount.toInt() - estimated));
      } else {
        txb.addOutput(receiverAddress, BigInt.from(amount));
      }
      if ((unspent.amount! - amount) != 0 && amount != availBal) {
        txb.addOutput(address,
            BigInt.from(((unspent.amount! - amount)).toInt() - estimated));
      }
      print("something");
      try {
        for (var i = 0; i < unspent.list!.length; i++) {
